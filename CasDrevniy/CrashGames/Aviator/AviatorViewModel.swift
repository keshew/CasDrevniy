...
class AviatorViewModel: ObservableObject {
    ...
    @Published var planePositionY: CGFloat = 0
    ...
    func startGame() {
        ...
        UserDefaultsManager.shared.incrementGamesPlayed(forGame: 5)
        ...
        let fallItem = DispatchWorkItem { [weak self] in
            ...
            withAnimation(.linear(duration: 2)) {
                self.planeRotation = 30
                self.planePositionX = 300
            }

            DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                UserDefaultsManager.shared.incrementWins(forGame: 5)
                UserDefaultsManager.shared.updateMaxWin(forGame: 5, winAmount: self.reward)
                self.reward = 0
